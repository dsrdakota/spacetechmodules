	...
	for(int i=0; i < nEdicts; i++)
	{
		...
		if(pEdict && !pEdict->IsFree())
		{
			...
			if(pEntity)
			{
				...
				if(AlwaysInstance || (FStrEq(ClassName.ToCStr(), "player") && pEntity->GetElasticity() != PVS_CONNECTED) || LUACheckTransmit(ClientIndex, EntIndex))
				{
